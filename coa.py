...
def coa(workers,managers,df1,df2):

    """ create a data frame for the final matchings """
    df_matching_coa = pd.DataFrame(index=workers, columns=[0, 1])
    for s in workers:
        df_matching_coa.loc[s][0] = s
    ...
    def C(m, p):
        ...
        df_matching_coa.loc[p][1] = m


    def D(x):
        matched_workers.remove(x)
        unmatched_workers.append(x)
        df_matching_coa.loc[x][1] = 'No match (Lost his match)'


    def current_match_of_accepter(m):
        q = ''.join(df_matching_coa.loc[df_matching_coa[1] == m][0].values)
        return q
            ...
            if p in unmatched_workers:
                if not pd.isnull(df1.loc[0][p]):  # p does not want to choose himself..
                    ...
                    if pd.isnull(rank_from___of___(m,p)):
                        ...
                        elif m is not df_matching_coa.loc[p][1]:
    ...
    """ Replace all NaN in df_matching_coa since they were never accepted"""
    df_matching_coa = df_matching_coa.replace(np.nan, 'NaN.', regex=True)

    """ Add the DA matchings to the total matchings"""

    story = story.reset_index()
    story = story.drop(['index'], 1)
    return df_matching_coa
