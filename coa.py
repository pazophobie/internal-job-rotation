...
def coa(workers,managers,df1,df2):
    ...
    coa_outcome = pd.DataFrame(index=workers, columns=[0, 1])
    for s in workers:
        coa_outcome.loc[s][0] = s
    ...
    def C(m, p):
        ...
        coa_outcome.loc[p][1] = m


    def D(x):
        matched_workers.remove(x)
        unmatched_workers.append(x)
        coa_outcome.loc[x][1] = 'No match (Lost his match)'


    def current_match_of_accepter(m):
        q = ''.join(coa_outcome.loc[coa_outcome[1] == m][0].values)
        return q
            ...
            if p in unmatched_workers:
                if not pd.isnull(df1.loc[0][p]):  # p does not want to choose himself..
                    ...
                    if pd.isnull(rank_from___of___(m,p)):
                        ...
                        elif m is not coa_outcome.loc[p][1]:
    ...
    """ Replace all NaN in coa_outcome since they were never accepted"""
    coa_outcome = coa_outcome.replace(np.nan, 'NaN.', regex=True)
